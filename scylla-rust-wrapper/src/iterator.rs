...
use crate::query_result::{
    cass_value_type, CassRawRow, CassRawValue, CassResult, CassResultKind, CassResultMetadata,
    CassRow, CassValue, NonNullDeserializationError,
};
...
pub struct CassListlikeIterator<'result> {
    ...
    value_data_type: Arc<CassDataType>,
    ...
    fn new_from_value(
        ...
        let item_type = match unsafe { value.value_type.get_unchecked() } {
            CassDataTypeInner::List { typ, .. } | CassDataTypeInner::Set { typ, .. } => {
                ...
                Arc::clone(typ.as_ref().expect(
                    "List or set type provided from result metadata should be fully typed!",
                ))
            }
            _ => {
    ...
    fn next(&mut self) -> bool {
        let next_value = self.iterator.next().and_then(|res| match res {
            Ok(value) => Some(CassValue {
                value,
                value_type: Arc::clone(&self.value_data_type),
            }),
            Err(e) => {
...
pub struct CassMapIterator<'result> {
    ...
    key_value_types: (Arc<CassDataType>, Arc<CassDataType>),
        ...
        let key_value_types = match unsafe { value.value_type.get_unchecked() } {
            CassDataTypeInner::Map { typ, .. } => match typ {
                MapDataType::KeyAndValue(key_type, val_type) => {
                    (Arc::clone(key_type), Arc::clone(val_type))
                }
        ...
        let new_entry = self
            ...
            .map(|(key, value)| {
                (
                    CassValue {
                        value: key,
                        value_type: Arc::clone(&self.key_value_types.0),
                    },
                    CassValue {
                        value,
                        value_type: Arc::clone(&self.key_value_types.1),
                    },
                )
            });
...
impl<'result> CassTupleIterator<'result> {
    ...
    fn next(&mut self) -> bool {
        ...
        let new_value = CassValue {
            value: raw_value,
            value_type: Arc::clone(&self.metadata[new_metadata_types_index]),
        };
...
impl<'result> CassUdtIterator<'result> {
    ...
    fn next(&mut self) -> bool {
        ...
        let new_value = CassValue {
            value: raw_value,
            value_type: Arc::clone(&self.metadata[new_metadata_types_index].1),
        };
        ...
        CassIterator::Collection(CassCollectionIterator::Map(map_collection_iterator)) => {
            map_collection_iterator
                ...
                .and_then(|state| {
                    ...
                    match state {
                        CassMapCollectionIteratorState::Key => {
                            current_entry.map(|entry| RefFFI::as_ptr(&entry.0))
                        }
                        CassMapCollectionIteratorState::Value => {
                            current_entry.map(|entry| RefFFI::as_ptr(&entry.1))
                        }
                    }
                })
                .unwrap_or(RefFFI::null())
...
pub unsafe extern "C" fn cass_iterator_get_user_type_field_name(
    ...
    let CassIterator::UdtFields(udt_iterator) = iter else {
    ...
    let iterator = match &result_from_raw.kind {
        ...
        CassResultKind::Rows(cass_rows_result) => {
            CassResultIterator::Rows(CassRowsResultIterator {
                ...
                iterator: cass_rows_result.raw_rows.rows_iter::<CassRawRow>().unwrap(),
...
pub unsafe extern "C" fn cass_iterator_fields_from_user_type<'result>(
    ...
    match iterator_result {
        Ok(iterator) => BoxFFI::into_ptr(Box::new(CassIterator::UdtFields(iterator))),
        Err(e) => {
