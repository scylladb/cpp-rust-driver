...
use scylla_cpp_driver::api::batch::{
    CassBatch, cass_batch_add_statement, cass_batch_new, cass_batch_set_consistency,
...
use crate::utils::{
    ...
    get_cass_consistency_from_rust_consistency, handshake_rules, proxy_uris_to_contact_points,
    setup_tracing, test_with_3_node_dry_mode_cluster,
};
...
fn check_for_all_consistencies_and_setting_options(
    ...
    unsafe {
        ...
        let mut cass_batch = {
            let mut cass_batch = cass_batch_new(std::mem::transmute(BatchType::Logged as u32));
            assert_cass_error_eq!(
            ...
            )
        {
            let [cass_c_cluster, cass_c_exec_profile] =
                [c_cluster, c_exec_profile].map(get_cass_consistency_from_rust_consistency);
            let [cass_sc_cluster, cass_sc_exec_profile] = [sc_cluster, sc_exec_profile].map(|sc| {
                get_cass_consistency_from_rust_consistency(match sc {
