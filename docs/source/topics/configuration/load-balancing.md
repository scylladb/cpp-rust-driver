...
Load balancing controls how queries are distributed to nodes in a Cassandra/ScyllaDB
cluster.
...
Cassandra/ScyllaDB clusters where all nodes are located in the same datacenter.
...
the Cassandra/ScyllaDB nodes where the data is located. Using this policy avoids having
to route requests through an extra coordinator node in the Cassandra/ScyllaDB cluster. This
can improve query latency and reduce load on the Cassandra/ScyllaDB nodes. It can be used
...
to poorly performing Cassandra/ScyllaDB nodes. It can be used in conjunction with other
