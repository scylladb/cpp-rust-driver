...
mod tests {
        ...
        unsafe {
            // untyped map (via cass_collection_new, Collection's data type is None).
            {
                ...
                assert_cass_error_eq!(
                    cass_collection_append_bool(untyped_map.clone(), false as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_int16(untyped_map.clone(), 42),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_double(untyped_map.clone(), 42.42),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_float(untyped_map.clone(), 42.42),
                    CassError::CASS_OK
                );
            ...
            // half-typed map (key-only)
            {
                ...
                assert_cass_error_eq!(
                    cass_collection_append_bool(half_typed_map.clone(), false as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_int16(half_typed_map.clone(), 42),
                    CassError::CASS_OK
                );

                // Second entry -> key typecheck failed.
                assert_cass_error_eq!(
                    cass_collection_append_double(half_typed_map.clone(), 42.42),
                    CassError::CASS_ERROR_LIB_INVALID_VALUE_TYPE
                );

                // Second entry -> typecheck succesful.
                assert_cass_error_eq!(
                    cass_collection_append_bool(half_typed_map.clone(), true as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_double(half_typed_map.clone(), 42.42),
            ...
            // typed map
            {
                ...
                let dt_ptr = ArcFFI::into_ptr(dt);
                let bool_to_i16_map = cass_collection_new_from_data_type(dt_ptr.clone(), 2);

                // First entry -> typecheck successful.
                assert_cass_error_eq!(
                    cass_collection_append_bool(bool_to_i16_map.clone(), false as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_int16(bool_to_i16_map.clone(), 42),
                    CassError::CASS_OK
                );

                // Second entry -> key typecheck failed.
                assert_cass_error_eq!(
                    cass_collection_append_float(bool_to_i16_map.clone(), 42.42),
                    CassError::CASS_ERROR_LIB_INVALID_VALUE_TYPE
                );

                // Third entry -> value typecheck failed.
                assert_cass_error_eq!(
                    cass_collection_append_bool(bool_to_i16_map.clone(), true as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_float(bool_to_i16_map.clone(), 42.42),
            ...
            // untyped set (via cass_collection_new, collection's type is None)
            {
                ...
                assert_cass_error_eq!(
                    cass_collection_append_bool(untyped_set.clone(), false as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_int16(untyped_set.clone(), 42),
                    CassError::CASS_OK
                );
            ...
            // typed set
            {
                ...
                let dt_ptr = ArcFFI::into_ptr(dt);
                let bool_set = cass_collection_new_from_data_type(dt_ptr.clone(), 2);

                assert_cass_error_eq!(
                    cass_collection_append_bool(bool_set.clone(), true as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_float(bool_set.clone(), 42.42),
                    CassError::CASS_ERROR_LIB_INVALID_VALUE_TYPE
                );
            ...
            // untyped list (via cass_collection_new, collection's type is None)
            {
                ...
                assert_cass_error_eq!(
                    cass_collection_append_bool(untyped_list.clone(), false as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_int16(untyped_list.clone(), 42),
                    CassError::CASS_OK
                );
            ...
            // typed list
            {
                ...
                let dt_ptr = ArcFFI::into_ptr(dt);
                let bool_list = cass_collection_new_from_data_type(dt_ptr.clone(), 2);

                assert_cass_error_eq!(
                    cass_collection_append_bool(bool_list.clone(), true as cass_bool_t),
                    CassError::CASS_OK
                );
                assert_cass_error_eq!(
                    cass_collection_append_float(bool_list.clone(), 42.42),
                    CassError::CASS_ERROR_LIB_INVALID_VALUE_TYPE
                );
    ...
    fn regression_empty_collection_data_type_test() {
        ...
        unsafe {
            ...
            cass_data_type_add_sub_type(empty_set_dt.clone(), empty_list_dt);
