...
pub enum CassRetryPolicy {
    ...
    LoggingRetryPolicy(Arc<CassLoggingRetryPolicy>),
}

impl RetryPolicy for CassRetryPolicy {
    fn new_session(&self) -> Box<dyn RetrySession> {
        match self {
            CassRetryPolicy::DefaultRetryPolicy(policy) => policy.new_session(),
            CassRetryPolicy::FallthroughRetryPolicy(policy) => policy.new_session(),
            CassRetryPolicy::DowngradingConsistencyRetryPolicy(policy) => policy.new_session(),
            CassRetryPolicy::LoggingRetryPolicy(policy) => policy.new_session(),
        }
    }
}
...
pub extern "C" fn cass_retry_policy_logging_new(
    ...
    ArcFFI::into_ptr(Arc::new(CassRetryPolicy::LoggingRetryPolicy(Arc::new(
