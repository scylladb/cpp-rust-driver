...
Variables also could be bound by name, where name could be inferred from query, or explicitly specified as `:name`. This limitation did exist because query metadata provided by Cassandra/ScyllaDB is required to map the variable name to the variable’s marker index.)
