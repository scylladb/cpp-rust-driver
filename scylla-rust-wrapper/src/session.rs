...
pub unsafe extern "C" fn cass_session_connect(
    session_raw: CassSharedBorrowedPtr<CassSession>,
...
pub unsafe extern "C" fn cass_session_prepare_from_existing(
    cass_session: CassSharedBorrowedPtr<CassSession>,
...
pub unsafe extern "C" fn cass_session_prepare(
    session: CassSharedBorrowedPtr<CassSession>,
    query: *const c_char,
) -> CassSharedOwnedPtr<CassFuture> {
...
pub unsafe extern "C" fn cass_session_prepare_n(
    cass_session_raw: CassSharedBorrowedPtr<CassSession>,
    query: *const c_char,
    query_length: size_t,
) -> CassSharedOwnedPtr<CassFuture> {
...
#[no_mangle]
pub unsafe extern "C" fn cass_session_free(session_raw: CassSharedOwnedPtr<CassSession>) {
...
mod tests {
    ...
    unsafe fn cass_future_wait_check_and_free(fut: CassSharedOwnedPtr<CassFuture>) {
        let fut_borrowed = fut.clone().into_borrowed();
        cass_future_wait(fut_borrowed.clone());
        if cass_future_error_code(fut_borrowed.clone()) != CassError::CASS_OK {
            let mut message: *const c_char = std::ptr::null();
            let mut message_len: size_t = 0;
            cass_future_error_message(fut_borrowed.clone(), &mut message, &mut message_len);
            eprintln!("{:?}", ptr_to_cstr_n(message, message_len));
        }
        assert_cass_error_eq!(
            cass_future_error_code(fut_borrowed.clone()),
            CassError::CASS_OK
        );
        ...
        unsafe {
            ...
            let profile_raw = cass_execution_profile_new();
            {
                cass_future_wait_check_and_free(cass_session_connect(
                    session_raw.clone().into_borrowed(),
                    cluster_raw.clone().into_const(),
                ));
                // Initially, the profile map is empty.

                assert!(ArcFFI::as_ref(&session_raw.clone().into_borrowed())
                    .unwrap()
                    .blocking_read()
                    .as_ref()
                    .unwrap()
                ...
                let profile_map_keys = ArcFFI::as_ref(&session_raw.clone().into_borrowed())
                    .unwrap()
                    .blocking_read()
                    .as_ref()
                    .unwrap()
    ...
    fn retry_policy_on_statement_and_batch_is_handled_properly_do(
        ...
        unsafe {
            ...
            cass_cluster_set_retry_policy(
                cluster_raw.clone(),
                fallthrough_policy.clone().into_borrowed(),
            );
            ...
            assert_cass_error_eq!(
                cass_execution_profile_set_retry_policy(
                    profile_raw.clone(),
                    default_policy.clone().into_borrowed()
                ),
                CassError::CASS_OK
            );
            ...
            ));
            {
                ...
                let set_retry_policy_on_stmt = |policy: CassSharedBorrowedPtr<CassRetryPolicy>| {
                    assert_cass_error_eq!(
                ...
                // F - F
                set_retry_policy_on_stmt(fallthrough_policy.clone().into_borrowed());
                assert_query_with_fallthrough_policy(&mut proxy);
                ...
                // F D D
                set_retry_policy_on_stmt(default_policy.clone().into_borrowed());
                assert_query_with_default_policy(&mut proxy);
                ...
                // F - D
                set_retry_policy_on_stmt(default_policy.into_borrowed());
                assert_query_with_default_policy(&mut proxy);
    ...
    fn session_with_latency_aware_load_balancing_does_not_panic() {
        unsafe {
            ...
            {
                let cass_future = cass_session_connect(
                    session_raw.clone().into_borrowed(),
                    cluster_raw.clone().into_const(),
                );
                cass_future_wait(cass_future.into_borrowed());
    ...
    rusty_fork_test! {
        ...
        fn cluster_is_not_referenced_by_session_connect_future() {
            ...
            unsafe {
                ...
                {
                    let cass_future = cass_session_connect(session_raw.clone().into_borrowed(), cluster_raw.clone().into_const());
