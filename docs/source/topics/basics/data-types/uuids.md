...
without requiring central coordination. These are often used in Cassandra/ScyllaDB
for primary and clustering keys. There are two types of UUIDs supported by
the driver (and Cassandra/ScyllaDB), version 1 which is time-based and version 4 which
is randomly generated. Version 1 can be used with Cassandra/ScyllaDB's `timeuuid` type
...
Version 4 can be used with Cassandra/ScyllaDB's `uuid` type for unique identification.
